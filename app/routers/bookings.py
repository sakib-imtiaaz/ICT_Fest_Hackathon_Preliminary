...
def create_booking(
    ...
    stats.record_create(db, room.id, price_cents)
...
def cancel_booking(
    ...
    _settlement_pause()
    updated = db.query(Booking).filter(Booking.id == booking_id, Booking.status == "confirmed").update({"status": "cancelled"})
    if updated == 0:
        db.rollback()
        raise AppError(409, "ALREADY_CANCELLED", "Booking already cancelled")
    db.commit()

    stats.record_cancel(db, booking.room_id, booking.price_cents)
    cache.invalidate_report(user.org_id)
